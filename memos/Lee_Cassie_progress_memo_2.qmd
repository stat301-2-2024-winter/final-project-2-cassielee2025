...
The 15,000 observations used for this prediction problem was split using a 75/25 split between training and testing data, stratified by birth weight. The dimensions of the data split are shown in @tbl-data-split.

```{r}
#| label: tbl-data-split
#| echo: false
#| tbl-cap: "Dimensions of data split"
...
The second distinct recipe operationalizes prenatal care as receiving prenatal care beginning in the first trimester or not, and creates a new variable to identify if mothers are in a higher risk age group (teenage or over 35). This should create a bigger emphasis on the age of the mother when they are in an age related risk group. Interaction terms were used between starting prenatal care early and number of prenatal visits, between weight gain and pre-pregnancy weight, between plural delivery and weight gain, between cigarettes and weight gain, and between interaction between risks and number of prenatal visits for linear based models. Similar to the other recipe, I removed variables wit exact linear correlations, predictors lacking any variance, and made appropriate adjustments to the recipe between recipes for linear and tree based models.
...
I have fit a null model and a standard linear regression using the basic recipe. The metrics of these models are shown in @tbl-null-linear-models:

```{r}
#| label: tbl-null-linear-models
#| echo: false
#| tbl-cap: "RMSE of null and linear model using basic recipe"
