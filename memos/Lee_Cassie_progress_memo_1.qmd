...
I had originally wanted to do a classification problem to predict whether or not an infant would be classified as being born with a low birth weight, however, upon inspection of the target variable, there were only about 10% of observations that were considered low birth weight.

This is interesting to me because I am interested in how environmental exposures related to climate change during windows of susceptibility affect birth weight. However, due to constraints on the data I was able to find, I am only focusing on the birth weight aspect for this prediction problem.
...
The target variable is brith weight, `dbwt`.

Aside from `dbwt`, there are 19 factor variables, 18 numeric variables, and 2 logical variables.
...
The target variable, `dbwt`, is pretty normally distributed, so there are not issues with skweness. No transformations will be needed to work with the target variable.

```{r}
#| label: target-variable-analysis

load(here("memos/memo1_outputs/target_variable.rda"))
plot
```
